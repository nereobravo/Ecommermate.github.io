...
//Porcion de codigo para que espere a que carguen todos los elementos del html antes de poner a funcionar el script
...
function guardarCarritoEnLocalStorage() {
    let carritoItems = document.getElementsByClassName('carrito-items')[0].getElementsByClassName('carrito-item');
    let carritoArray = [];

    for (let i = 0; i < carritoItems.length; i++) {
        let item = carritoItems[i];
        let titulo = item.querySelector('.carrito-item-titulo').innerText;
        let precio = parseFloat(item.querySelector('.carrito-item-precio').innerText.replace('$', '').replace('.', ''));
